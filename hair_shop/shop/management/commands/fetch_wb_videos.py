...
def convert_to_h264(input_path: str) -> bool:
    ...
    result = subprocess.run(
        [
            ...
            "fast",  # скорость
            "-movflags",
            "+faststart",  # для быстрого старта в браузере
...
class Command(BaseCommand):
    ...
    def _process_category(
        self, category_id: int, options: dict
    ) -> tuple[int, int, int]:
        ...
        for i, (article, url) in enumerate(video_urls.items(), start=1):
            self.stdout.write(f"[{i}/{len(video_urls)}] {article}", ending="\r")
            self.stdout.flush()
            ...
            # Шаг 1: скачиваем
            self.stdout.write(
                f"[{i}/{len(video_urls)}] скачиваю:    {article}", ending="\r"
            )
            self.stdout.flush()
            ...
            self.stdout.write(
                f"[{i}/{len(video_urls)}] перекодирую: {article}", ending="\r"
            )
            self.stdout.flush()
        ...
        self.stdout.write("")
        self.stdout.write(
            f"Категория {category_id}: скачано {ok}, ошибок {errors}, пропущено {skip}"
        )
