...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Найдено в БД: {len(file_articles - not_in_db)} из {len(file_articles)}"
        )

        if not_in_db:
            self.stdout.write(
                self.style.WARNING(f"Не найдено в БД: {len(not_in_db)} артикулов")
            )
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING("DRY RUN — запись в БД не производится")
            )
        ...
        for _, row in df.iterrows():
            ...
            product.hair_length = (
                str(raw_length).strip()
                if pd.notna(raw_length) and str(raw_length).strip()
                else ""
            )
            ...
            product.hair_width = (
                str(raw_width).strip()
                if pd.notna(raw_width) and str(raw_width).strip()
                else None
            )

            raw_strands = row.get("number_of_strands")
            product.number_of_strands = (
                str(raw_strands).strip()
                if pd.notna(raw_strands) and str(raw_strands).strip()
                else ""
            )
        ...
        if missing_articles and not dry_run:
            ...
            self.stdout.write(
                self.style.WARNING(f"Пропущенные артикулы сохранены → {missing_path}")
            )
