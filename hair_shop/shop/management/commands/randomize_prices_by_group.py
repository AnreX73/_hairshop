...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if num_groups_param is not None:
            ...
            self.stdout.write(
                f"Используем виртуальные группы: {num_groups} групп (случайное распределение товаров)."
            )
            # Случайно назначаем каждой товару номер группы от 0 до num_groups-1
            group_ids = [
                random.randint(0, num_groups - 1) for _ in range(total_products)
            ]
            ...
            group_values = list(
                Product.objects.exclude(product_group__isnull=True)
                .values_list("product_group", flat=True)
                .distinct()
            )
            num_groups = len(group_values)
            self.stdout.write(
                f"Найдено уникальных значений product_group: {num_groups}"
            )
            ...
            if num_groups > total_products * 0.2:
                ...
                if proceed.lower() != "y":
                    self.stdout.write(
                        "Отменено. Запустите с параметром --num-groups <желаемое_количество_групп>."
                    )
                    return
        ...
        self.stdout.write(
            f"Групп со скидкой: {len(groups_with_discount)} ({len(groups_with_discount) / total_groups * 100:.1f}%)"
        )
        ...
        if dry_run:
            ...
            for group_products, price, discount in group_data:
                ...
                self.stdout.write(
                    f"Группа из {count} товаров (id: {sample}...): price={price}, discount={discount}%"
                )
            return
        ...
        with transaction.atomic():
            for group_products, price, discount in group_data:
                ...
                rows = Product.objects.filter(id__in=product_ids).update(
                    price=price, discount_percentage=discount
                )
                updated_total += rows
                self.stdout.write(
                    f"Обновлено {rows} товаров: price={price}, discount={discount}%"
                )
